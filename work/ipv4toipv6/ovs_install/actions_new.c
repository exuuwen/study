...
static unsigned int ovs_ipv4v6_key = 0;
static struct ctl_table_header *ovs_sysctl_header;
static unsigned int ovs_ipv4_to_ipv6 __read_mostly = 1;

static ctl_table ovs_table[] = {
	{
		.procname	= "ovs_ipv4_to_ipv6",
		.data		= &ovs_ipv4_to_ipv6,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
	},
	{
		.procname	= "ovs_ipv4v6_key",
		.data		= &ovs_ipv4v6_key,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
...
#define NET_OVS 21
static struct ctl_path ovs_path[] = {
	{ .procname = "net", .ctl_name = CTL_NET, },
	{ .procname = "ovs", .ctl_name = NET_OVS, },
	{ }
};


int ovs_ipv4v6_key_init(void)
{
	ovs_sysctl_header = register_sysctl_paths(ovs_path, ovs_table);
	if (ovs_sysctl_header == NULL) {
		printk(KERN_WARNING
		       "ovs: can't register to sysctl.\n");
		return -ENOMEM;
	}
...
void ovs_ipv4v6_key_exit(void)
{
	unregister_sysctl_table(ovs_sysctl_header);
}
...
static int new_sample(struct sk_buff *skb)
{
	...
	if (type == 1)
	{
		...
		v6_key_be =  htonl(ovs_ipv4v6_key);
		...
		*(__be32 *)hdr = htonl(0x60000000);
		hdr->payload_len = htons(tot_len + (is_frag ? sizeof(struct frag_hdr) : 0));
		hdr->hop_limit = ttl;
		...
		else if (proto == IPPROTO_ICMPV6)
		{
			...
			if (skb->len - sizeof(struct ipv6hdr) < tot_len)
				return -1; 

			icmph->icmp6_cksum = 0;
			skb->csum = skb_checksum(skb, sizeof(struct ipv6hdr), tot_len, 0);
	...
	else if (type == 2)
	{
		...
		tot_len = ntohs(hdr->payload_len);
		dstip_be = daddr.s6_addr32[2];
		if (ovs_ipv4v6_key)
			srcip_be = saddr.s6_addr32[2];
		else
		...
		if (proto == IPPROTO_FRAGMENT)
		{
			...
			proto = fh->nexthdr;
			if (tot_len > sizeof(struct frag_hdr))
		...
		iph->tot_len = htons(tot_len + sizeof(struct iphdr));
		ip_send_check(iph);
		...
		else if (proto == IPPROTO_ICMP)
		{
			...
			if (skb->len - sizeof(struct iphdr) < tot_len)
				return -1;

			icmph->checksum = 0;
			skb->csum = skb_checksum(skb, sizeof(struct iphdr), tot_len, 0);
