...
ip netns exec ns1 ip l a link eth0 name vlan type vlan id 100
ip netns exec ns1 ifconfig eth0 up
ip netns exec ns1 ifconfig vlan 10.0.0.1/24 up
...
bridge vlan add dev veth1 vid 100 pvid
...
#ifconfig br-test 10.0.0.3/24 up
#bridge vlan add dev br-test vid 100 pvid untagged self  # set br-test port must take with self


# vid: receive vlan tag
# pvd: if no tag or tag_id ==0, set the tage_id to vid
# untagged: send out through the vethx, untagged the vlan header
