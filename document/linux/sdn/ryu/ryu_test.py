...
mac = ("00:1e:65:15:fc:01", "00:1e:65:15:fc:02")
...
class Simple(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.add_flow(datapath, 60010, settings.UINT32_MAX, match, actions)

    def add_flow(self, datapath, priority, buffer_id, match, actions):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def _packet_in_handler(self, ev):
        ...
        self.logger.info("packet in %s %s %s %s %s", dpid, src, dst, in_port, buffer_id)
        ...
        if eth.ethertype == ether.ETH_TYPE_ARP:
            ...
            if _arp.opcode == arp.ARP_REQUEST and _arp.src_ip != "0.0.0.0" and _arp.src_ip != _arp.dst_ip:
                if ip2mac.has_key(_arp.dst_ip):
                    ...
                    flow = "priority=60020,arp,dl_dst=ff:ff:ff:ff:ff:ff,dl_src="+src+",nw_dst="+_arp.dst_ip+",actions=move:NXM_OF_ETH_SRC[]->NXM_OF_ETH_DST[],mod_dl_src:"+arp_tha+",load:0x2->NXM_OF_ARP_OP[],move:NXM_NX_ARP_SHA[]->NXM_NX_ARP_THA[],move:NXM_OF_ARP_SPA[]->NXM_OF_ARP_TPA[],set_field:"+arp_tha+"->arp_sha,set_field:"+_arp.dst_ip+"->arp_spa,in_port"
                    print flow
                    call(["ovs-ofctl", "add-flow", "br0", flow, "-O", "OpenFlow13"])
                    return
        elif dst in mac and src in mac: 
            ...
            if in_port == 1:
                ...
                return 
            #time.sleep(1)
            self.logger.info("ping packet.....")
            actions.append(parser.OFPActionOutput(port))
            self.add_flow(datapath, 0, buffer_id, match, actions)
