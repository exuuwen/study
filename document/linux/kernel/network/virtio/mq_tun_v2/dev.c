...
static int netdev_rx_queue_update_kobjects(struct net_device *dev, int old_num, int new_num)
{
	int i;
	int error = 0;
	...
	while (--i >= new_num)
		kobject_put(&netdev_extended(dev)->rps_data._rx[i].kobj);
 ...
 *	succeeds.
 */
void netif_set_real_num_rx_queues(struct net_device *dev, unsigned int rxq)
{
	int rc;
	...
	if (dev->reg_state == NETREG_REGISTERED) {
		...
		rc = netdev_rx_queue_update_kobjects(dev, netdev_extended(dev)->rps_data.real_num_rx_queues,
						  rxq);
		if (rc)
			return;
	}

	netdev_extended(dev)->rps_data.real_num_rx_queues = rxq;
	return;
}
		       ...
		       struct rps_dev_flow **rflowp)
{
	...
	if (skb_rx_queue_recorded(skb)) {
		...
		if (unlikely(index >= rpinfo->real_num_rx_queues)) {
			WARN_ONCE(rpinfo->real_num_rx_queues > 1, "%s received packet "
				"on queue %u, but number of RX queues is %u\n",
				dev->name, index, rpinfo->real_num_rx_queues);
			goto done;
		}
...
int register_netdevice(struct net_device *dev)
{
	...
	if (!netdev_extended(dev)->rps_data.num_rx_queues) {
		...
		netdev_extended(dev)->rps_data.real_num_rx_queues = 1;
		...
		unsigned int txqs, unsigned int rxqs)
{
	...
	netdev_extended(dev)->rps_data.real_num_rx_queues = rxqs;
