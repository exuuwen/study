...
static int childFunc(void *arg)          
{
    ...
    sprintf(nboxaddr, "192.168.130.%d/24", nid);
    ...
    if (create_route(NULL, "192.168.130.2", "eth0") < 0)
        errExit("fail create route1");
    if (verbose)
        printf("create 192.168.130.2  through eth0\n");
    
    if (create_route("192.168.130.2", NULL, "eth0") < 0)
        errExit("fail create route2");
    if (verbose)
        printf("create default route with gw 192.168.130.2 through eth0\n");
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("user %d already create a nbox with 192.168.130.%d\n", uid, nid);
        return 0; 
    }
    ...
    if (bridge_add_port(nbox_veth, "br0"))
        errExit("nbox veth bridge_add_port");
    if (verbose)
        printf("bridge_add_port nbox veth to br0\n");
    ...
    sprintf(ssh_con, "ssh -X 192.168.130.%d", nid);
    system(ssh_con);
