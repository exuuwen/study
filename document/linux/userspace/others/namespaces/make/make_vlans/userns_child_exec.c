...
static int childFunc(void *arg)          
{
    ...
    if (nid == 1 || nid == 2 || nid == 5 || nid == 6)
        sprintf(nboxaddr, "10.0.10.%d/24", nid);
    else if (nid == 3 || nid == 4 || nid == 7 || nid == 8)
        sprintf(nboxaddr, "10.0.20.%d/24", nid);
    ...
    if (nid == 1 || nid == 2 || nid == 5 || nid == 6)
    {
        if (create_route("10.0.10.11", NULL, "eth0") < 0)
        	errExit("fail create route1");
    }
    else if (nid == 3 || nid == 4 || nid == 7 || nid == 8)
    {
        if (create_route("10.0.20.11", NULL, "eth0") < 0)
        	errExit("fail create route1");
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("already create a nbox with 192.168.130.%d\n", nid);
        return 0; 
    }
    ...
    child_pid = clone(childFunc, child_stack + STACK_SIZE, flags | SIGCHLD, &uid);
    if (child_pid == -1)
        errExit("clone");
    ...
    if (nid == 1 || nid == 2)
    {
        if (bridge_add_port(nbox_veth, "vlanbr1"))
            errExit("nbox veth bridge_add_port");
        if (verbose)
            printf("bridge_add_port nbox veth to br1\n");
    }
    else if (nid == 3 || nid == 4)
    {
        if (bridge_add_port(nbox_veth, "vlanbr2"))
            errExit("nbox veth bridge_add_port");
        if (verbose)
            printf("bridge_add_port nbox veth to br2\n");
    }
    else if (nid == 5 || nid == 6)
    {
        if (bridge_add_port(nbox_veth, "vlanbr11"))
            errExit("nbox veth bridge_add_port");
        if (verbose)
            printf("bridge_add_port nbox veth to br11\n");
    }
    else if (nid == 7 || nid == 8)
    {
        if (bridge_add_port(nbox_veth, "vlanbr22"))
            errExit("nbox veth bridge_add_port");
        if (verbose)
            printf("bridge_add_port nbox veth to br22\n");
    }
    else
    {
       assert(0);
    ...
    if (nid == 1 || nid == 2 || nid == 5 || nid == 6)
        sprintf(ssh_con, "ssh -X 10.0.10.%d", nid);
    else if (nid == 3 || nid == 4 || nid == 7 || nid == 8)
        sprintf(ssh_con, "ssh -X 10.0.20.%d", nid);
