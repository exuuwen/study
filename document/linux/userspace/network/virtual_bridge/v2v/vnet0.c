...
./vnet0 vnet0 1 192.168.1.7 
Set 'vnet0' persistent:1
set vnet0 ip addr:192.168.1.7
...
int create_packet(unsigned char *buf, int len)
{
	...
	*/
	...
	//memcpy(mac_header->h_source, buf1.ifr_hwaddr.sa_data, ETH_ALEN);
	/*
	bzero(&buf1, sizeof(buf1));
	strcpy(buf1.ifr_name, "eth0"); 
	if (ioctl(s, SIOCGIFHWADDR, &buf1)) 
	{
		perror("SIOCGIFHWADDR");
		close(s);
	...
	*/
	memcpy(mac_header->h_dest, packet_dst, ETH_ALEN);//dest can be other any one of the mac of the ports in the bridge, so it can be forward
	...
	ip_header->daddr = inet_addr("192.168.1.8");

	ip_header->saddr = inet_addr("192.168.1.7"); 
	ip_header->check = 0;
