...
static void do_recvmsg(int fd, struct sockaddr *addr)
{
	int ret;
	struct info buf;
	struct iovec iovec;
	...
	/*Shan! you also can the addr which you want to receive*/
	...
	while(1){
		...
		ret = recvmsg(fd, &datagrams, 0);
		if (ret < 0) {
			perror("sendmmsg");
			exit(1);
		}
		printf("recv buf.num:%d. buf.Id: %c\n", buf.num, buf.id);
		/*Shan! you can check the buf here*/	
...
{
	fprintf(stderr, "Usage: recvmmsg_test  <host> <port>  \n");
}
